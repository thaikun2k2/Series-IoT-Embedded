...
void loop() { 
  ...
  int doamdat = map(soilMoisture, 0, 4095, 0, 100);//(1024 - soilMoisture) / 1023.0 * 100;
  ...
  if(Mode4 == 0) {  
    ...
    if (hour == hourOnRelay4 && minute == minuteOnRelay4) {
      digitalWrite(Relay4_PIN, HIGH);
    } else if (hour <= hourOffRelay4 && minute >= minuteOffRelay4) {
      digitalWrite(Relay4_PIN, LOW);
    }
  }
  ...
  if(Mode3 == 0) {
    ...
    if (hour == hourOnTimerCheNang && minute == hourOffTimerCheNang) {
      digitalWrite(CheNang_PIN, LOW);
    } else if (hour == hourOffTimerCheNang && minute >= minuteOffTimerCheNang) {
      digitalWrite(CheNang_PIN, HIGH);
    }else {
      digitalWrite(CheNang_PIN, HIGH);
    }
  ...
  if(Mode2 == 0) {
    ...
    if (temperature <= temperatureOn) {
      ...
      digitalWrite(Relay3_PIN, LOW);
        ...
        // Timer Light
    //Timer Fan
    if (hour == hourOnRelay3 && minute == minuteOnRelay3) {
      digitalWrite(Relay3_PIN, HIGH);
    } else if (hour == hourOffRelay3 && minute >= minuteOffRelay3) {
      digitalWrite(Relay3_PIN, LOW);
    }
  }
  ...
  if (Mode1 == 0) {
    ...
    if(Firebase.RTDB.getBool(&fbdo, "/Device/Relay2")) {
      ...
      Serial.println("FAILED: /Device/Relay2" + fbdo.errorReason());
        ...
        // Timer Light
    if (hour == hourOnRelay2 && minute == minuteOnRelay2) {
      digitalWrite(Relay2_PIN, HIGH);
    } else if (hour == hourOffRelay2 && minute >= minuteOffRelay2) {
